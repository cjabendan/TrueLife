...
export const createPost = mutation({
    args: {
        ...
        const postId = await ctx.db.insert("posts", {
            ...
            // incremenet user's post count by 1
...
export const deletePost = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        // verify ownership
        if (post.userId !== currentUser._id) throw new Error("Not authorized to delete this posts.");
...
export const getLikedPosts = query({
    handler: async (ctx) => {
        ...
        const likedPostsWithInfo = await Promise.all(
        ...
        return likedPostsWithInfo.filter(Boolean);
    },
});